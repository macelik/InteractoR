...
import_db <- function(db_type = c("noncurated", "curated", "both")) {
  ...
  if (db_type == "both") {
    ...
    combined_db <- combined_db[!duplicated(combined_db$pair), ]

    print("Number of pairs found")
    return(table(combined_db$annotation_strategy))
...
#' pairwise_interactions <- create_pairwise_pairs(both_db)
...
#'
#' @examples
#' # Assuming 'complete_data' is your data frame with Ligand and Receptor columns
#' annotated_data <- annotate_components(complete_data)
#'
#' @export
...
process_lr_db <- function(complete, annotation, all_ligands = NULL, all_receptors = NULL) {
    ...
    return(rbind(true_LR_DB, LR_fix_dir))
}
